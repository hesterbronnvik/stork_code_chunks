...
wind <- thermals %>% 
  ...
  ggplot(aes(avg_wind_speed, fill = as.factor(journey_number), group = as.factor(journey_number))) +
  geom_density(alpha = 0.5) +
  labs(x = "Wind speed (m/s)", y = "Density", fill = "Migration") +
  scale_fill_manual(values = colfunc(3)) +
  facet_wrap(~lat_bin, ncol = 1)
radius <- thermals %>% 
  mutate(lat_bin = ifelse(location.lat > 42.5, "Central Europe",
                          ifelse(between(location.lat, 36, 42.5), "Spain",
                                 ifelse(between(location.lat, 27.4, 36), "Morocco",
                                        ifelse(location.lat < 15, "Sahel", "Sahara")))),
         lat_bin = factor(lat_bin, levels = c("Central Europe", "Spain", "Morocco", "Sahara", "Sahel"))) %>% 
  ggplot(aes(avg_rad, fill = as.factor(journey_number), group = as.factor(journey_number))) +
  geom_density(alpha = 0.5) +
  labs(x = "Circling radius (m)", y = "Density", fill = "Migration") +
  scale_fill_manual(values = colfunc(3)) +
  facet_wrap(~lat_bin, ncol = 1)
turn <- thermals %>% 
  mutate(lat_bin = ifelse(location.lat > 42.5, "Central Europe",
                          ifelse(between(location.lat, 36, 42.5), "Spain",
                                 ifelse(between(location.lat, 27.4, 36), "Morocco",
                                        ifelse(location.lat < 15, "Sahel", "Sahara")))),
         lat_bin = factor(lat_bin, levels = c("Central Europe", "Spain", "Morocco", "Sahara", "Sahel"))) %>% 
  ggplot(aes(log(turn_var_thermal), fill = as.factor(journey_number), group = as.factor(journey_number))) +
  geom_density(alpha = 0.5) +
  labs(x = "log turning variance (deg/s)^2", y = "Density", fill = "Migration") +
  scale_fill_manual(values = colfunc(3)) +
  facet_wrap(~lat_bin, ncol = 1)
vs <- thermals %>% 
  mutate(lat_bin = ifelse(location.lat > 42.5, "Central Europe",
                          ifelse(between(location.lat, 36, 42.5), "Spain",
                                 ifelse(between(location.lat, 27.4, 36), "Morocco",
                                        ifelse(location.lat < 15, "Sahel", "Sahara")))),
         lat_bin = factor(lat_bin, levels = c("Central Europe", "Spain", "Morocco", "Sahara", "Sahel"))) %>% 
  ggplot(aes(sqrt(vspeed_thermal), fill = as.factor(journey_number), group = as.factor(journey_number))) +
  geom_density(alpha = 0.5) +
  labs(x = "Vertical speed (m/s)", y = "Density", fill = "Migration") +
  scale_fill_manual(values = colfunc(3)) +
  facet_wrap(~lat_bin, ncol = 1)
# png(filename = "/home/hbronnvik/Documents/chapter2/figures/look24/poster/data_vis/densities.png",
#     height = 8.2*1.5, width = 11.7*1.5, units = "in", res = 500)
ggpubr::ggarrange(vs, wind, turn, radius, ncol = 4, common.legend = T, legend = "right")
# dev.off()

dat <- readRDS(list.files("/home/hbronnvik/Documents/chapter2/wind_thermal_data/", pattern = "2024-01-16|2024-01-17", full.names = T)[[10]])

for (i in c(19, 69, 73, 76)) {
  t1 <- dat %>% 
    filter(thermal_event %in% unique(thermal_event)[i])
  png(filename = paste0("/home/hbronnvik/Documents/chapter2/figures/look24/poster/thermal_", i, ".png"),
      height = 8.3, width = 8.3, units = "in", res = 500)
  plot3D::scatter3D(t1$location.long, t1$location.lat, t1$height_above_ellipsoid, 
                    colvar = t1$CircRadius, col = colfunc(135), alpha = 0.75, pch=16,bty="g", nticks=5, 
                    ticktype="detailed", theta=-45, phi=20, xlab="Longitude", ylab="Latitude", zlab="Height",
                    clab = "Circling \nradius (m)")
  dev.off()
}
...
turn_df <- classified %>% 
  ...
  mutate(turn_direction = ifelse(turn.angle > 0, "counter", "clock"),
         ...
         n.changes = length(unique(directional_set))) %>% 
  ungroup() %>% 
  dplyr::select(timestamp, location.long, thermal_event, turn_direction, directional_change,
                directional_set, n.changes)

turn_df <- turn_df %>% 
  group_by(thermal_event) %>% 
  slice(1) %>% 
  ungroup() %>% 
  dplyr::select(thermal_event, n.changes)

hist(turn_df$n.changes, breaks= 100, main = "", col = "gray40")
b <- MASS::boxcox(lm(turn_df$n.changes ~ 1))
b$x[which.max(b$y)]
lambda <- b$x[which.max(b$y)]
turn_df$boxcox_n.changes <- ((turn_df$n.changes^lambda - 1)/lambda)
hist(turn_df$boxcox_n.changes, breaks= 100, main = "", col = "gray40")
...
model_data <- thermals %>% 
  ...
  mutate_at(c("location.lat","ld_day", "sqrt_vspeed", "log_turn_var_thermal", 
              "avg_wind_speed", "avg_rad", "avg_bank", "journey_number", "boxcox_n.changes"), list(z = ~(scale(.)))) %>% 
...
# interaction
plots <- lapply(c("log_turn_var_thermal", "boxcox_n.changes", "sqrt_vspeed"), function(v){

    if(v == "log_turn_var_thermal"){
      mod_yr3 <- lme4::lmer(log_turn_var_thermal_z ~ journey_number_z*location.lat_z*avg_wind_speed_z + 
                                (1 | individual.id), 
                              data = model_data, REML = F)
      vee <- "turning variance (deg/s)^2"
    }
    if(v == "boxcox_n.changes"){
      mod_yr3 <- lme4::lmer(boxcox_n.changes_z ~ journey_number_z*location.lat_z*avg_wind_speed_z + 
                                (1 | individual.id), 
                              data = model_data, REML = F)
      vee <- "number of directional changes"
    }
    if(v == "sqrt_vspeed"){
      mod_yr3 <- lme4::lmer(sqrt_vspeed_z ~ journey_number_z*location.lat_z*avg_wind_speed_z + 
                                (1 | individual.id), 
                              data = model_data, REML = F)
      vee <- "vertical speed (m/s)" 
    }
    
    conf_yr3 <- confint(mod_yr3) %>% 
      as.data.frame() %>% 
      rownames_to_column(var = "pred") %>%
      left_join(lme4::fixef(mod_yr3) %>% 
                  as.data.frame() %>% 
                  rename(fixef = ".") %>% 
                  rownames_to_column(var = "pred"), 
                by = join_by(pred)) %>% 
      ...
      rename(lower = "2.5 %",
             upper = "97.5 %") %>% 
      slice(4:n())  %>% 
      mutate(pred = gsub("avg_wind_speed_z", "Wind speed", 
                         gsub("journey_number_z", "Migrations", 
                              gsub("location.lat_z", "Latitude",
                                   pred))),
             pred = factor(pred, levels = c("Latitude", "Wind speed", "Migrations",
                                            "Migrations:Latitude", "Migrations:Wind speed", "Latitude:Wind speed", 
                                            "Migrations:Latitude:Wind speed")))
    
    
    yr_coefs <- ggplot(conf_yr3, aes(fixef, forcats::fct_rev(pred))) +
      geom_vline(xintercept = 0, lty = 2, color = "gray40") +
      geom_pointrange(aes(xmin = lower, xmax = upper), color = "#007BA7") +
      labs(x = paste0("Effect on ", vee, " +/- se"), y = "", title = "")
    
    # grd_yr <- expand.grid(journey_number = 1:3,
    #                        avg_wind_speed = seq(from = quantile(model_data$avg_wind_speed, 0.025, na.rm = T), 
    #                                             to = quantile(model_data$avg_wind_speed, 0.975, na.rm = T), 
    #                                             length.out = 100),
    ...
    #          boxcox_n.changes = mean(model_data$boxcox_n.changes, na.rm = T),
    #          sqrt_vspeed = mean(model_data$sqrt_vspeed, na.rm = T),
    #          interaction = "age_wind_map")
    grd_yr <- expand.grid(journey_number = 1:3,
                          avg_wind_speed = seq(from = quantile(model_data$avg_wind_speed, 0.025, na.rm = T), 
                                               to = quantile(model_data$avg_wind_speed, 0.975, na.rm = T), 
                                               length.out = 100)) %>%
      mutate(log_turn_var_thermal = mean(model_data$log_turn_var_thermal, na.rm = T),
             boxcox_n.changes = mean(model_data$boxcox_n.changes, na.rm = T),
             sqrt_vspeed = mean(model_data$sqrt_vspeed, na.rm = T),
             location.lat = mean(model_data$location.lat, na.rm = T),
    ...
    new_data_only <- model_data %>%
      ...
      mutate(log_turn_var_thermal_z = (log_turn_var_thermal - mean(model_data$log_turn_var_thermal))/(sd(model_data$log_turn_var_thermal)),
             ...
             sqrt_vspeed_z = (sqrt_vspeed - mean(model_data$sqrt_vspeed))/(sd(model_data$sqrt_vspeed)),
    ...
    preds <- predict(mod_yr3, newdata = new_data, type = "link")
    ...
    if(v == "sqrt_vspeed"| v == "log_turn_var_thermal"){
      colfunc <- colorRampPalette(c("#0098b0", "#00AFB9", "#7fc4b8", "#FED9B7", "#f7a58f", "#f27e71", "#F07167", "#ee5e53"))
    }
    if(v == "boxcox_n.changes"){
      colfunc <- colorRampPalette(c("#0081A7", "#0098b0", "#00AFB9", "#7fc4b8", "#FED9B7", "#f7a58f", "#ee5e53"))
    }
    
    yr_tile <- inter_preds %>%
      filter(interaction == "age_wind") %>% 
      ggplot(aes(journey_number, avg_wind_speed, fill = preds)) +
      geom_raster(interpolate = F) +
      scale_y_continuous(expand = c(0, 0), breaks = c(2, 4, 6, 8)) +
      # scale_x_continuous(expand = c(0, 0), breaks = seq(1, 30, 3)) +
      labs(x = "Migration", y = "Wind speed (m/s)", title = " ") +
      scale_fill_gradientn("Prediction", colors = colfunc(10))
    
    p <- ggpubr::ggarrange(yr_coefs, yr_tile + theme(legend.position = "right"), 
                           nrow = 1, widths = c(2, 1))
    return(p)
})

for (i in 1:length(plots)) {
  png(filename = paste0("/home/hbronnvik/Documents/chapter2/figures/look24/poster/model_res/age_winds3_",i,".png"),
      height = 5.5, width = 24, units = "in", res = 800)
  print(plots[[i]])
  dev.off()
}
...
tiled <- inter_preds %>% 
  mutate(journey_number = ifelse(journey_number == 1, "Migration 1 (n = 137)", 
                                  ifelse(journey_number == 2, "Migration 2 (n = 27)", 
                                         "Migration 3 (n = 21)"))) %>% 
  ggplot(aes(location.lat, avg_wind_speed, fill = probs)) +
  geom_raster(interpolate = F) +
  scale_y_continuous(expand = c(0, 0), breaks = 1:10) +
  scale_x_reverse(expand = c(0, 0), breaks = c(45, 40, 35, 30, 25, 20)) +
  labs(y = "Wind speed (m/s)", x = "Latitude", title = "") +
  scale_fill_gradientn("Trend", colors = colfunc(10)) +
  facet_wrap(~journey_number)

png(filename = paste0("/home/hbronnvik/Documents/chapter2/figures/look24/poster/turn_mod3.png"),
    height = 8.3, width = 11.7, units = "in", res = 500)
ggpubr::ggarrange(coefs3, tiled, nrow = 2)
dev.off()
...
mod_circ3 <- lme4::lmer(avg_rad_z ~ journey_number_z*location.lat_z + 
                           (1 | individual.id), 
                         data = model_data, REML = F)
# AIC      BIC   logLik deviance df.resid 
# 100694.6 100745.6 -50341.3 100682.6    36234 
conf_circ_3 <- confint(mod_circ3) %>% 
  as.data.frame() %>% 
  rownames_to_column(var = "pred") %>%
  left_join(lme4::fixef(mod_circ3) %>% 
              as.data.frame() %>% 
              rename(fixef = ".") %>% 
              rownames_to_column(var = "pred"), 
            by = join_by(pred)) %>% 
...
circ <- ggplot(conf_circ_3, aes(fixef, forcats::fct_rev(pred))) +
  geom_vline(xintercept = 0, lty = 2, color = "gray40") +
  geom_pointrange(aes(xmin = lower, xmax = upper), color = "#007BA7") +
  labs(x = "Effect on circling radius +/- se", y = "Predictor")

grd_lat <- expand.grid(journey_number = (1:max(model_data$journey_number)),
                       location.lat = seq(from = quantile(model_data$location.lat, 0.025, na.rm = T), to = quantile(model_data$location.lat, 0.975, na.rm = T), length.out = 100)) %>%
  mutate(boxcox_n.changes = mean(model_data$boxcox_n.changes, na.rm = T),
         avg_wind_speed = mean(model_data$avg_wind_speed, na.rm = T),
         avg_rad = mean(model_data$avg_rad, na.rm = T),
         interaction = "age_map")
...
new_data_only <- model_data %>%
  ...
  bind_cols(grd_lat) %>% 
  #calculate z-scores. get the mean(center) and sd(scale) from the previous z transformation. for consistency
  mutate(avg_rad_z = (avg_rad - mean(model_data$avg_rad))/(sd(model_data$avg_rad)),
         journey_number_z = (journey_number - mean(model_data$journey_number))/(sd(model_data$journey_number)),
         location.lat_z = (location.lat - mean(model_data$location.lat, na.rm = T))/(sd(model_data$location.lat, na.rm = T)),
         avg_wind_speed_z = (avg_wind_speed - mean(model_data$avg_wind_speed, na.rm = T))/(sd(model_data$avg_wind_speed, na.rm = T)))
...
# now that we have the values to predict, run the model on them
preds <- predict(mod_circ3, newdata = new_data, type = "link")
...
map_tile <- inter_preds %>% 
  filter(interaction == "age_map") %>% 
  # mutate(journey_number = ifelse(journey_number == 1, "Migration 1 (n = 137)", 
  #                                ifelse(journey_number == 2, "Migration 2 (n = 27)", 
  #                                       "Migration 3 (n = 21)"))) %>% 
  ggplot(aes(journey_number, location.lat, fill = preds)) +
  geom_raster(interpolate = F) +
  scale_y_continuous(expand = c(0, 0), breaks = c(45, 40, 35, 30, 25, 20)) +
  labs(y = "Latitude", x = "Migration", title = "") +
  scale_fill_gradientn("Prediction", colors = colfunc(10))

# png(filename = paste0("/home/hbronnvik/Documents/chapter2/figures/look24/poster/circ_mod2.png"),
#     height = 8.3, width = 11.7, units = "in", res = 500)
ggpubr::ggarrange(circ, map_tile, nrow = 1)
# dev.off()


mod_clock3 <- lme4::lmer(boxcox_n.changes_z ~ journey_number_z*location.lat_z*avg_wind_speed_z + 
                                (1 | individual.id), 
                              data = model_data, REML = F)
# AIC      BIC   logLik deviance df.resid 
# 74886.9  74971.2 -37433.5  74866.9    33877 
conf_limit_clock_3 <- confint(mod_clock3) %>% 
  as.data.frame() %>% 
  rownames_to_column(var = "pred") %>%
  left_join(lme4::fixef(mod_clock3) %>% 
              as.data.frame() %>% 
              rename(fixef = ".") %>% 
              rownames_to_column(var = "pred"), 
            by = join_by(pred)) %>% 
  ...
  mutate(pred = gsub("avg_wind_speed", "Wind speed", 
                     gsub("location.lat", "Latitude", 
                          gsub("journey_number", "Migrations", 
                               gsub("_z", "", pred)))),
         pred = factor(pred, levels = c("Latitude", "Wind speed", "Migrations", "Migrations:Latitude",
                                         "Migrations:Wind speed", "Latitude:Wind speed",
                                        "Migrations:Latitude:Wind speed"))
  )

clock <- ggplot(conf_limit_clock_3, aes(fixef, forcats::fct_rev(pred))) +
  geom_vline(xintercept = 0, lty = 2, color = "gray40") +
  geom_pointrange(aes(xmin = lower, xmax = upper), color = "#007BA7") +
  labs(x = "Effect on number of directional changes +/- se", y = "Predictor")
...
grd_wind <- expand.grid(journey_number = (1:max(model_data$journey_number)),
                        avg_wind_speed = seq(from = quantile(model_data$avg_wind_speed, 0.025, na.rm = T), to = quantile(model_data$avg_wind_speed, 0.975, na.rm = T), length.out = 100)) %>%
  mutate(boxcox_n.changes = mean(model_data$boxcox_n.changes, na.rm = T),
         location.lat = mean(model_data$location.lat, na.rm = T),
         interaction = "age_wind")

grd_wind_map <- bind_rows(grd_lat, grd_wind) 
...
new_data_only <- model_data %>%
  ...
  # only keep the columns that I need
  dplyr::select(c("individual.id")) %>% 
  bind_cols(grd_wind_map) %>% 
  #calculate z-scores. get the mean(center) and sd(scale) from the previous z transformation. for consistency
  mutate(boxcox_n.changes_z = (boxcox_n.changes - mean(model_data$boxcox_n.changes))/(sd(model_data$boxcox_n.changes)),
         journey_number_z = (journey_number - mean(model_data$journey_number))/(sd(model_data$journey_number)),
         location.lat_z = (location.lat - mean(model_data$location.lat, na.rm = T))/(sd(model_data$location.lat, na.rm = T)),
         avg_wind_speed_z = (avg_wind_speed - mean(model_data$avg_wind_speed, na.rm = T))/(sd(model_data$avg_wind_speed, na.rm = T)))
...
# now that we have the values to predict, run the model on them
preds <- predict(mod_clock3, newdata = new_data, type = "link")
...
colfunc <- colorRampPalette(c("#0081A7", "#0098b0", "#00AFB9", "#7fc4b8", "#FED9B7", "#f27e71"))
...
wind_tile <- inter_preds %>% 
  ...
  labs(y = "Wind speed (m/s)", x = "Migration", title = "") +
  scale_fill_gradientn("Prediction", colors = colfunc(10))

row2 <- ggpubr::ggarrange(map_tile, wind_tile, nrow = 1, common.legend = T, legend = "right")
# png(filename = paste0("/home/hbronnvik/Documents/chapter2/figures/look24/poster/clock_mod3.png"),
#     height = 8.3, width = 11.7, units = "in", res = 500)
ggpubr::ggarrange(clock, row2, nrow = 2)
# dev.off()
...
### ld_days
plots <- lapply(c("log_turn_var_thermal", "boxcox_n.changes", "sqrt_vspeed"), function(v){
  part_plot <- lapply(split(model_data, model_data$journey_number), function(ld){
    ld <- ld %>% 
      dplyr::select(timestamp, location.long, location.lat, individual.id, ld_day, m_day, t_burst, 
                    migration, journey_number, trackID, ind_burst_id, thermal_event, track_status, season, 
                    sqrt_vspeed, log_turn_var_thermal, avg_wind_speed, avg_rad, avg_bank) %>% 
    ...
    if(v == "log_turn_var_thermal"){
      mod_days3 <- lme4::lmer(log_turn_var_thermal_z ~ ld_day_z*avg_wind_speed_z + 
                                     (1 | individual.id), 
                                   data = ld, REML = F)
      vee <- "turning variance (deg/s)^2"
      }
    if(v == "boxcox_n.changes"){
        mod_days3 <- lme4::lmer(boxcox_n.changes_z ~ ld_day_z*avg_wind_speed_z + 
                                       (1 | individual.id), 
                                     data = ld, REML = F)
        vee <- "number of directional changes"
    }
    if(v == "sqrt_vspeed"){
      mod_days3 <- lme4::lmer(sqrt_vspeed_z ~ ld_day_z*avg_wind_speed_z + 
                                       (1 | individual.id), 
                                     data = ld, REML = F)
      vee <- "vertical speed (m/s)" 
    }
    ...
    # AIC      BIC   logLik deviance df.resid 
    # 60284.0  60333.3 -30136.0  60272.0    27222 
    conf_days3 <- confint(mod_days3) %>% 
      as.data.frame() %>% 
      rownames_to_column(var = "pred") %>%
      left_join(lme4::fixef(mod_days3) %>% 
                  as.data.frame() %>% 
                  rename(fixef = ".") %>% 
                  rownames_to_column(var = "pred"), 
                by = join_by(pred)) %>% 
      ...
      # filter(pred != "journey_number_z") %>%  # this has no real meaning and can't be interpreted
      mutate(pred = gsub("avg_wind_speed", "Wind speed", 
                         gsub("ld_day", "Day of migration", 
                              gsub("journey_number", "Migrations", 
                                   gsub("_z", "", pred)))),
             pred = factor(pred, levels = c("Day of migration", "Wind speed", "Day of migration:Wind speed")))
    

    day_turn <- ggplot(conf_days3, aes(fixef, forcats::fct_rev(pred))) +
      geom_vline(xintercept = 0, lty = 2, color = "gray40") +
      geom_pointrange(aes(xmin = lower, xmax = upper), color = "#007BA7") +
      labs(x = paste0("Effect on ", vee, " +/- se"), y = "", title = paste0("Migration ", unique(ld$journey_number)))
    
    grd_day <- expand.grid(ld_day = seq(from = quantile(ld$ld_day, 0.025, na.rm = T), 
                                        to = quantile(ld$ld_day, 0.975, na.rm = T), 
                                        length.out = 25),
                           avg_wind_speed = seq(from = quantile(ld$avg_wind_speed, 0.025, na.rm = T), 
                                                to = quantile(ld$avg_wind_speed, 0.975, na.rm = T), 
                                                length.out = 25)) %>%
      mutate(log_turn_var_thermal = mean(ld$log_turn_var_thermal, na.rm = T),
             boxcox_n.changes = mean(ld$boxcox_n.changes, na.rm = T),
             sqrt_vspeed = mean(ld$sqrt_vspeed, na.rm = T),
             location.lat = mean(ld$location.lat, na.rm = T),
             interaction = "days_wind")
    ...
    new_data_only <- ld %>%
      ...
      bind_cols(grd_day) %>% 
      #calculate z-scores. get the mean(center) and sd(scale) from the previous z transformation. for consistency
      mutate(log_turn_var_thermal_z = (log_turn_var_thermal - mean(ld$log_turn_var_thermal))/(sd(ld$log_turn_var_thermal)),
             boxcox_n.changes_z = (boxcox_n.changes - mean(ld$boxcox_n.changes))/(sd(ld$boxcox_n.changes)),
             sqrt_vspeed_z = (sqrt_vspeed - mean(ld$sqrt_vspeed))/(sd(ld$sqrt_vspeed)),
             ld_day_z = (ld_day - mean(model_data$ld_day))/(sd(ld$ld_day)),
             location.lat_z = (location.lat - mean(ld$location.lat, na.rm = T))/(sd(model_data$location.lat, na.rm = T)),
             avg_wind_speed_z = (avg_wind_speed - mean(ld$avg_wind_speed, na.rm = T))/(sd(ld$avg_wind_speed, na.rm = T)))
    ...
    # now that we have the values to predict, run the model on them
    preds <- predict(mod_days3, newdata = new_data, type = "link")
    ...
    if(unique(ld$journey_number) != 2){
      colfunc <- colorRampPalette(c("#0081A7", "#0098b0", "#00AFB9", "#7fc4b8", "#FED9B7", "#f7a58f", "#f27e71", "#F07167", "#ee5e53"))
    }else{
      colfunc <- colorRampPalette(c("#0081A7", "#0098b0", "#00AFB9", "#7fc4b8", "#FED9B7", "#f7a58f", "#F07167"))
    }
    
    day_tile <- inter_preds %>% 
      filter(interaction == "days_wind") %>% 
      ggplot(aes(ld_day, avg_wind_speed, fill = preds)) +
      geom_raster(interpolate = F) +
      scale_y_continuous(expand = c(0, 0), n.breaks = 10) +
      scale_x_continuous(expand = c(0, 0), breaks = seq(1, 30, 3)) +
      labs(x = "Day of migration", y = "Wind speed (m/s)", title = " ") +
      scale_fill_gradientn("Prediction", colors = colfunc(10))
    
    p <- ggpubr::ggarrange(day_turn, day_tile + theme(legend.position = "right"), nrow = 1, widths = c(2, 1))
    return(p)
  })
})
# png(filename = paste0("/home/hbronnvik/Documents/chapter2/figures/look24/poster/turn_day2.png"),
#     height = 8.3*1.5, width = 11.7*1.5, units = "in", res = 500)
ggpubr::ggarrange(plots[[1]][[1]], plots[[1]][[2]], plots[[1]][[3]], nrow = 3)
# dev.off()
# png(filename = paste0("/home/hbronnvik/Documents/chapter2/figures/look24/poster/model_res/clock_day2.png"),
#     height = 8.3*1.5, width = 11.7*1.5, units = "in", res = 500)
ggpubr::ggarrange(plots[[2]][[1]], plots[[2]][[2]], plots[[2]][[3]], nrow = 3)
# dev.off()
# png(filename = paste0("/home/hbronnvik/Documents/chapter2/figures/look24/poster/model_res/vspeed_day2.png"),
#     height = 8.3*1.5, width = 11.7*1.5, units = "in", res = 500)
ggpubr::ggarrange(plots[[3]][[1]], plots[[3]][[2]], plots[[3]][[3]], nrow = 3)
# dev.off()
