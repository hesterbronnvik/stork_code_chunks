...
## Get all the relevant info regarding individuals & migrations
...
## Get the flight GPS and the ACC data
classified <- lapply(list.files("/home/hbronnvik/Documents/chapter2/wind_thermal_data/250deg_20s/", full.names = T), readRDS)
...
# the 149 birds relevant to the study
classified <- unique(classified$individual.id)

classified <- info %>% 
  filter(number_of_events > 0) %>% 
  mutate(timestamp_end = as.POSIXct(sub("\\..*", "", timestamp_end), tz = "UTC")) %>% 
  filter(individual_id %in% classified) %>% 
  dplyr::select(individual_id, study)

acc_data <- lapply(1:nrow(classified), function(x){
  acc_df <- getMovebankNonLocationData(classified$study[x], "Acceleration",
                                       classified$individual_id[x], loginStored)
  saveRDS(acc_df, file = paste0("/home/hbronnvik/Documents/chapter2/acc_data/", classified$individual_id[x],
                                "_acc_2023-10-29.rds"))
})

# GPS flight data
gps_files <- list.files("/home/hbronnvik/Documents/chapter2/wind_thermal_data/250deg_20s", full.names = T)#[grepl("173673348|173661590|23460438", list.files("/home/hbronnvik/Documents/chapter2/classified_data", full.names = T))]
...
# all the ACC data
acc_files <- list.files("/home/hbronnvik/Documents/chapter2/acc_data_24", full.names = T)
acc_birds <- lapply(acc_files, function(f){
  gsub("_1024.rds", "", 
       gsub("/home/hbronnvik/Documents/chapter2/acc_data_24/", "", f))
}) %>% unlist() %>% paste(collapse = "|")

# birds that gave both GPS and ACC
gps_files <- gps_files[grepl(acc_birds, gps_files)]

start_time <- Sys.time()
flight_acc <- lapply(c("Fall", "Spring"), function(season){
  half <- lapply(1:length(gps_files), function(x){
    print(x)
    # load the GPS data
    ...
    rec <- records %>% 
      filter(individual.id == unique(gex$individual.id) & season == season)
    ...
    if(nrow(rec) > 0){
      ...
      if(nrow(gex) > 0){
        ...
        acc_remaining <- aex %>% 
          filter(flight == T) %>% 
          mutate(date = date(timestamp)) %>% 
          left_join(rec[, c("date", "ld_day", "season")], by = join_by(date)) %>% 
          left_join(meta[, c("trackID", "migration")], by = join_by(trackID)) 
        return(acc_remaining)
      }
    }
  })
  half <- half[!sapply(half, function(x) is.null(x))]
  half <- data.table::rbindlist(half, fill = T, use.names = T)
  return(half)
})
flight_acc <- data.table::rbindlist(flight_acc)
Sys.time()-start_time # Time difference of 19.37883 mins (fall only)
# Time difference of 1.00348 hours (spring and fall)
# flight_acc <- flight_acc[!sapply(flight_acc, function(x) is.null(x))]

flight_acc <- split(flight_acc, flight_acc$individual_id)

# saveRDS(flight_acc, file = "/home/hbronnvik/Documents/chapter2/flight_acc_20240814.rds")

flight_acc <- readRDS(file = "/home/hbronnvik/Documents/chapter2/flight_acc_20240814.rds")

transform_to_g <- function(x, slope){(x-2048)*slope}

flight_DBA <- lapply(1:length(flight_acc), function(a){
  print(a)
  acc <- flight_acc[[a]] 
  acc <- acc %>% as.data.frame()
  ...
  slope <- ifelse(unique(acc$tag_local_identifier) > 10000, 1/1024, 
                  ifelse(unique(acc$tag_local_identifier) < 4118, 0.0022, 1/512))
  ...
  VeDBA <- lapply(1:nrow(acc), function(x){
    ...
    raw_accMx <- matrix(as.integer(unlist(strsplit(as.character(acc[x, accRawCol]), " "))), ncol=3, byrow = T)
    # raw_accMx %>% 
    #   as.data.frame() %>% 
    #   mutate(r = 1:n()) %>%
    #   pivot_longer(cols = c(contains("V"))) %>% 
    #   ggplot(aes(r, value, color = name)) +
    #   geom_path()
    # # and convert to units of g
    accMx <- transform_to_g(raw_accMx, slope = slope)
    # accMx %>% 
    #   as.data.frame() %>% 
    #   mutate(r = 1:n()) %>%
    #   pivot_longer(cols = c(contains("V"))) %>% 
...
flight_DBA %>% 
  ...
  labs(x = "residual VeDBA", y = "Density", fill = "Migration") +
  scale_fill_manual(values = colfunc(3)) +
  facet_grid(cols = vars(lat_bin), rows = vars(season))
...
flapping <- lapply(1:length(flight_acc), function(a){
  ...
  if(nrow(acc) > 5){
    ...
    # transform the data to g 
    # slope <- ifelse(unique(acc$tag_local_identifier) > 10000, 1/1024,
    #                 ifelse(unique(acc$tag_local_identifier) < 4118, 0.0022, 1/512))
    #     # rowwise go through the raw values
    # acc_t <- lapply(1:nrow(acc), function(x){
    #   raw_accMx <- matrix(as.integer(unlist(strsplit(as.character(acc[x, "eobs_accelerations_raw"]), " "))), ncol=3, byrow = T)
    #   accMx <- transform_to_g(raw_accMx, slope = slope)
    #   row_made <- lapply(1:nrow(accMx), function(i){
    #     paste0(accMx[i,], collapse = " ")
    #   }) %>%
    #     unlist() %>%
    #     paste0(collapse = " ")
    #   accelerationTransformed2 <- row_made
    #   return(accelerationTransformed2)
    # }) %>% reduce(rbind)
    # acc$accelerationTransformed2 <- acc_t
    ...
    # saveRDS(wingbeatsDF, file = paste0("/home/hbronnvik/Documents/chapter2/flapping_flight/", ID, "_270527.rds"))
    return(wingbeatsDF)
  }
}) %>% reduce(rbind)
...
flapping <- flapping %>% 
  dplyr::select(-burstID) %>% 
  left_join(flight_DBA %>% group_by(individual_id, timestamp) %>% slice(1), 
            by = join_by(timestamp, individual_id)) %>% 
  rename(burst_id = burstID)

# saveRDS(flapping, file = "/home/hbronnvik/Documents/chapter2/flapping_20240814.rds")

flapping <- readRDS("/home/hbronnvik/Documents/chapter2/flapping_20240814.rds")
...
ggplot(flapping, aes(behavior, dba_res, fill = behavior)) +
  geom_boxplot() +
  # geom_jitter(width = 0.25, alpha = 0.05) +
  scale_fill_manual(values = c("#334371", "#6279B8")) +
  labs(x = "Classification (Individuals = 137, n = 64672)", y = "Difference between VeDBA and its per-tag mean") +
...
classified <- data.table::rbindlist(classified, use.names = T, fill = T) %>% 
  ...
  dplyr::select(individual.id, timestamp, location.long, location.lat, journey_number, season, 
                ...
                wind_direction, cross_wind, wind_support, n_switches, exit_height, height_above_ellipsoid)
gc()

flapping <- lapply(list.files("/home/hbronnvik/Documents/chapter2/flapping_flight", pattern = "_270527", full.names = T), function(file){
  readRDS(file) %>% 
...
flight_wind_acc <- classified %>% 
  ...
  left_join(flapping %>% 
              dplyr::select(-location.long, -location.lat, -ld_day), 
            by = join_by(individual.id, season, trackID, burst_id))

summary(flight_wind_acc$VeDBA)

ggplot(flight_wind_acc %>% drop_na(behavior), aes(sqrt(mean_speed))) +
  geom_histogram(bins = 100, color = "black") +
  labs(x = "sqrt mean wind speed") +
...
flight_wind_acc %>% 
  ...
  facet_wrap(~journey_number+season) +
  theme(legend.position = "none")
# dev.off()
...
flapping_count <- flapping %>%
  ...
  slice(1) %>% 
  ungroup() %>% 
  dplyr::select(date, individual.id, flap_ratio)
...
# png(filename = "/home/hbronnvik/Documents/chapter2/figures/look24/acc/daily_flapping_fob2.png",
#     width = 11.5, height = 8.5/2, units = "in", res = 300)
p_mort <- thermal_other %>%
  group_by(individual.id, date) %>% 
  slice(1) %>% 
...
ggplot(p_mort, aes(log(fall_in_ratio), log(flap_ratio))) +
  geom_vline(xintercept = mean(log(p_mort$fall_in_ratio)), lty = 2) +
  geom_hline(yintercept = mean(log(p_mort$flap_ratio)), lty = 2) +
  geom_point(aes(color = mort), cex = .5) +
  ...
  facet_wrap(~season+journey_number)
# dev.off()
...
log_graph <- summary(log_model)[[10]] %>% 
  as.data.frame() %>% 
  cbind(confint(log_model, method = "Wald") %>% as.data.frame() %>% slice(2:n()))
...
png(filename = "/home/hbronnvik/Documents/chapter2/figures/look24/acc/12.5_flapping_windAge.png",
