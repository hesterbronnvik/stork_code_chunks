...
n_alt <- 50 #number of alternative steps. (create more than are strictly needed to remove some later)
...
full_ml <- readRDS("/home/hbronnvik/Documents/chapter3/migration_locations_40km70km15daySpeed_2025-03-08.rds")
...
full_ml <- full_ml %>% 
  ...
  filter(id_date %in% metad$id_date & ground_speed_15 > 2)
# arrange to the satisfaction of the move object
full_ml <- full_ml %>% 
  arrange(individual.id, timestamp) %>% 
  as.data.frame() #%>% 
  # filter(individual.id %in% unique(individual.id)[1:2])
# make a move object
...
if(length(sp_obj_ls) > 0){
  ...
  png(paste0("/home/hbronnvik/Documents/chapter3/figures/", hr, "_", tolerance, ".png"),
      height = (8.2/3)*2, width = 11.7, units = "in", res = 500)
  par(mfrow=c(1,2))
  hist(sl, freq=F, main="", xlab = "Step length (km)", breaks = 50, col = "gray50")
  plot(function(x) dgamma(x, shape = fit.gamma1$estimate[[1]],
                          rate = fit.gamma1$estimate[[2]]), add = TRUE, from = 0.1, to = 150, 
       col = "#0081A7", lwd = 3)
  hist(rad(burst_df$turning_angle[complete.cases(burst_df$turning_angle)]),
       freq=F,main="",xlab="Turning angles (radians)", breaks = 50, col = "gray50")
  plot(function(x) dvonmises(x, mu = mu, kappa = kappa), add = TRUE, from = -3.5, to = 3.5, 
       col = "#EE5E53", lwd = 3)
  dev.off()

  #diagnostic plots for step length distribution
  png(paste0("/home/hbronnvik/Documents/chapter3/figures/", hr, "_", tolerance, "_diag.png"),
  ...
  used_av_track <- lapply(1:length(sp_obj_ls), function(x){ #for each trackment
    print(x)
    track <- sp_obj_ls[[x]]
    track <- split(track, track$burst_id)
    ttls <- lapply(track,function(burst){ #for each burst,
      ...
      if(kappa != Inf){
        ...
        bbls <- lapply(c(2:(length(burst)-1)), function(this_point){ #first point has no bearing to calc turning angle, last point has no used endpoint.
          ...
          df <- used_point@data %>%  
            ...
            mutate(location.long = c(head(x,1),rnd_sp@coords[,1]), #the coordinates were called x and y in the previous version
                   ...
                   # convert turning angles into the range used by move objects (i.e. used locations)
                   turning_angle = c(head(turning_angle,1),((deg(rnd_sp$turning_angle))+ 180) %% 360 - 180),
        ...
        bbdf <- data.table::rbindlist(bbls)
        return(bbdf)
      }else{
          df <- burst@data %>% 
            mutate(used = NA,
    ...
    ttdf <- data.table::rbindlist(ttls)
    # saveRDS(ttdf, file = paste0("/home/hbronnvik/Documents/chapter3/burst_tracks/", ttdf$track[1], ".rds"))
    return(ttdf)
  })
}
  
Sys.time() - b # Time difference of 1.98634 hours

# used_av_track <- lapply(list.files("/home/hbronnvik/Documents/chapter3/burst_tracks", full.names = T), function(f){
#   f %>% 
#     readRDS() %>% 
...
used_av_df <- data.table::rbindlist(used_av_track)

#create one dataframe with fewer variables
used_av_df <- used_av_df %>%
  dplyr::select(timestamp, location.lat, location.long, selected, individual.id,  
                burst_id, step_length, turning_angle, track, step_id, used, heading) %>%
  # mutate(timestamp = paste(as.character(timestamp),"000",sep = ".")) %>%
  mutate(stratum = paste(track, burst_id, step_id, sep = "_")) %>% #create unique stratum id
  as.data.frame()

# saveRDS(used_av_df, file = "/home/hbronnvik/Documents/chapter3/used_av_df_60min.rds")
