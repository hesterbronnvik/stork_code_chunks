...
studies <- c(10449318, 2204484313, 24442409) # Movebank study IDs
...
# full metadata for 86 individuals that sent GPS data
...
lapply(74:nrow(info), function(x){
  print(x)
...
full_files <- list.files("/home/hbronnvik/Documents/WS_data/GPS_data", full.names = T, pattern = "20250516")
# the date the data were retrieved from Movebank
dwld_date <- "2025-05-16"
...
clean_locations <- lapply(1:length(full_files), function(x){
  ...
  # consider eastern birds
  if(max(locs_df$location.long) >= 16.5){
...
}) # Time difference of of 6.609327 mins
Sys.time() - start_time

# n <- 2 # to test a single ID
start_time <- Sys.time()
...
Sys.time()-start_time # Time difference of 5.521832 secs
...
meta <- migration_locations %>%
  ...
  ungroup()
# saveRDS(meta, file = "/home/hbronnvik/Documents/chapter3/metadata_eastern_tracks_2025-05-16.rds")
...
migration_locations <- migration_locations %>% 
  ...
  # consider only eastern migrants
...
# saveRDS(migration_locations, file = paste0("/home/hbronnvik/Documents/chapter3/eastern_migration_locations_40km70km15daySpeed_", Sys.Date(),".rds"))

# migration_locations <- readRDS("/home/hbronnvik/Documents/chapter3/eastern_migration_locations_40km70km15daySpeed_2025-05-16.rds")
...
# ggplot(data = world) +
#   geom_sf(fill = "gray80", color = "gray80") +
#   coord_sf(xlim = c(-15, 50), ylim = c(-33, 55), expand = T) +
#   geom_point(data = migration_locations, aes(location.long, location.lat, color = track_status),
#              alpha = 0.25)
...
# sources and sinks of Eastern birds
sources <- migration_locations %>% 
  left_join(meta %>% dplyr::select(trackID, journey_number, total_journeys, season)) %>% 
  filter(track_status == "complete" & season == "fall" & journey_number == 1) %>% 
  group_by(trackID) %>% 
  arrange(timestamp) %>% 
  slice(1) %>% 
  ungroup() %>% 
...
# write.table(sources, file = "/home/hbronnvik/Documents/chapter3/Circuit/dem_sources_east.txt",
#             col.names = F, quote = T, sep = "\t")
# write.table(sinks, file = "/home/hbronnvik/Documents/chapter3/Circuit/dem_grounds_east.txt",
#             col.names = F, quote = T, sep = "\t")
